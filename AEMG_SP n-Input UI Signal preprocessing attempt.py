...
class SerialReader:
    ...
    def rectify(self, data):
        return [np.abs(val) for val in data]
        ...
        while not self.stop_thread:
            try:
                if self.ser.inWaiting():
                    ...
                    if len(raw_data) != self.num_sensors + 1:  # expect timestamp + n sensor values
                        raise ValueError
                    # Apply rectification to sensor data
                    rectified_data = [raw_data[0]] + self.rectify(raw_data[1:])
                    self.data_queue.put(rectified_data)
