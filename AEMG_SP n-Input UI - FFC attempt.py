...
# ! FFC stands for Feed Forward Comb Filter
...
FILTER_DELAY = 13  # Delay for Comb Filter (in testing 24 seems to be the best)
FILTER_GAIN = -1  # Gain for Comb Filter (in testing 1 seems to be the best)
AXIS_HEIGHT = 700 # Height, both +/- for y-axis
...
class CombFilter:
    ...
    def filter(self, signal):
        output = signal + self.gain * self.buffer[self.delay - 1]
...
class DataPlotter:
    def __init__(self, reader: SerialReader, plot_window: int = 500) -> None:
        ...
        self.comb_filters = [CombFilter(delay=FILTER_DELAY, gain=FILTER_GAIN) for _ in range(reader.num_sensors)]  # Initialize Comb Filters
        self.setup_ui()
        self.setup_plot()
    ...
    def toggle_y_axis(self) -> None:
        if self.is_auto_scaled:
            for p in self.plot:
                p.setYRange(min=-AXIS_HEIGHT, max=AXIS_HEIGHT)
