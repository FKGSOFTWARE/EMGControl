...
class ThresholdNotifier:
    def __init__(self, reader: SerialReader, delay: float = 0.5, queue_size: int = 20) -> None:
        self.reader = reader
        self.delay = delay
        self.stop_thread = False
        self.value_queues = [collections.deque(maxlen=queue_size) for _ in range(reader.num_sensors)]
        self.thread = Thread(target=self.check_conditions)
        self.thread.start()
...
if __name__ == "__main__":
    ...
    notifier = ThresholdNotifier(reader)
