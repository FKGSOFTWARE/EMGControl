...
import Breakout_modified_updated as game
...
class DataPlotter:
    ...
    def setup_ui(self) -> None:
        ...
        self.sensor1_extension = State("Extension", 0.0)  # Initialize with 0.0 as threshold
        ...
        self.sensor2_flexion = State("Flexion", 0.0)  # Initialize with 0.0 as threshold
    ...
    def update(self) -> None:
        ...
        while not self.reader.data_queue.empty():
            ...
            for i, value in enumerate(data[1:]):
                ...
                self.curve[i].setData(self.time_data, abs(self.value_data[i]))

            try:
                update_msg = threshold_queue.get_nowait()
                if update_msg["type"] == "Extension":
                    self.state_manager1.update_threshold("Extension", update_msg["value"])
                elif update_msg["type"] == "Flexion":
                    self.state_manager2.update_threshold("Flexion", update_msg["value"])
            except queue.Empty:
                # No updates in the queue, continue with other tasks or sleep for a bit
                pass

    def start(self) -> None:
        QApplication.instance().exec_()

def put_control_value(value) -> None: 
    with queue_lock:
...
def greater_of_two_states(a, b) -> State:
    # print("value a: " + str(a[1]) + ", value b: " + str(b[1]))
    if a[1] > b[1]:
        return a
    else:
        return b
...
def start_game() -> None:
    game.run_game(control_queue, threshold_queue)
