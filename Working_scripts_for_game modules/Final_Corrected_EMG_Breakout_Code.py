...
class DataPlotter:
    ...
    def update(self) -> None:
        ...
        while not self.reader.data_queue.empty():
            data = [abs(val) for val in self.reader.data_queue.get()]
            self = self.shift_and_append(self, abs(data)[0])
            if self.is_recording:
                with self.record_lock:
                    self.record_abs(data).append(abs(data))
            for i, value in enumerate(abs(data)[1:]):
